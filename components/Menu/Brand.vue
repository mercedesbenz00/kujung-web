...
import { BRAND_SUBMENU_LIST } from '~~/ts/constants';
...
<template>
    <el-collapse-transition>
        <div class="relative flex bg-warmGray-400 bg-opacity-30 lg:rounded-br-[200px]" @mouseover="store.setIsBrand(true);"
            @mouseleave=" store.setIsBrand(false);">
            <div class="lg:hidden">
                <i class="absolute right-0 top-0 las la-times self-end text-[25px] md:text-[35px] font-bold p-[15px] md:p-[30px] cursor-pointer"
                    @click="store.setIsBrand(false)"></i>
            </div>

            <!-- 메뉴 사진 부분(PC) -->
            <div class="pl-[132px] pr-[50px] py-[40px] bg-[#c2bfb54d] hidden lg:block">
                <img :src="BRAND_SUBMENU_LIST[selectedSubMenuIdx].img" width="372" height="220" />
                <div class="pl-1">
                    <p class="text-body1 mt-[24px] mb-[50px]">{{ BRAND_SUBMENU_LIST[selectedSubMenuIdx].title }}</p>
                    <p class="text-body2">{{ BRAND_SUBMENU_LIST[selectedSubMenuIdx].desc }}</p>
                </div>
            </div>

            <!-- 메뉴 텍스트 부분 -->
            <div class="px-[30px] md:px-[50px] py-[40px] flex flex-col justify-between gap-[78px] w-full">
                <div class="flex gap-[56px] h-[350px] ">
                    <el-tabs tab-position="left" class="menu-tabs">
                        <template v-for="(  subMenu, index  ) in   BRAND_SUBMENU_LIST  ">
                            <el-tab-pane>
                                <template #label>
                                    <span
                                    ...
                                    <ul
                                        ...
                                        <template v-for="  child   in   subMenu.children  ">
                                            <li class="text-body1 cursor-pointer hover:text-primary-400"
                                                @click=" onItemClick(`/brand/${subMenu.title_route}/${child.title_route}`)">
                                                {{ child.title }}
                                            </li>
                                        </template>
                                    </ul>
                ...
                <div
                    ...
                    <img :src="BRAND_SUBMENU_LIST[selectedSubMenuIdx].img" class="w-[160px] md:w-[200px] h-[94px] md:h-[120px]">
                    <div class="ml-[14px] md:ml-[34px]">
                        <p class="text-[15px] md:text-[18px] leading-[20px] md:leading-[24px] mb-[10px]">{{
                            BRAND_SUBMENU_LIST[selectedSubMenuIdx].title }}</p>
                        <p class="text-[13px] md:text-[15px] leading-[18px] md:leading-[20px]">{{
                            BRAND_SUBMENU_LIST[selectedSubMenuIdx].desc }}</p>
                    </div>
                </div>
