...
import { SMARTSERVICE_SUBMENU_LIST } from '~~/ts/constants';
...
const onItemClick = (p_navRoute: string) => {
    props.onHideMenu()
    if (p_navRoute.includes("http")) {
        window.open(p_navRoute.substring(p_navRoute.indexOf("http")), '_blank');
    } else {
...
const onSubtitleTitleClick = (index: number) => {
    if (SMARTSERVICE_SUBMENU_LIST[index].children.length > 0) {
        return
    }
    if (hovering.value) {
    ...
    onItemClick(`/smartservice/${SMARTSERVICE_SUBMENU_LIST[index].title_route}`)
}
...
<template>
    <el-collapse-transition>
        <div class="relative flex bg-warmGray-400 bg-opacity-30 lg:rounded-br-[200px]"
            @mouseover="store.setIsSmartService(true);" @mouseleave="store.setIsSmartService(false)">
            <div class="lg:hidden">
                <i class="absolute right-0 top-0 las la-times self-end text-[25px] md:text-[35px] font-bold p-[15px] md:p-[30px] cursor-pointer"
                    @click="store.setIsSmartService(false)"></i>
            </div>

            <!-- LG 메뉴 사진 부분 -->
            <div class="pl-[132px] pr-[50px] py-[40px] bg-[#c2bfb54d] hidden lg:block">
                <img :src="SMARTSERVICE_SUBMENU_LIST[selectedSubMenuIdx].img" width="372" height="220" />
                <div class="pl-1">
                    <p class="text-body1 mt-[24px] mb-[50px]">{{ SMARTSERVICE_SUBMENU_LIST[selectedSubMenuIdx].title }}</p>
                    <p class="text-body2">{{ SMARTSERVICE_SUBMENU_LIST[selectedSubMenuIdx].desc }}</p>
                </div>
            </div>
            ...
            <div class="px-[30px] md:px-[50px] py-[40px] flex flex-col justify-between gap-[78px]">
                <div class="flex gap-[56px] h-[350px] ">
                    <el-tabs tab-position="left" class="menu-tabs">
                        <template v-for="(subMenu, index) in SMARTSERVICE_SUBMENU_LIST">
                            <el-tab-pane>
                                <template #label>
                                ...
                                <div class="flex" v-if="SMARTSERVICE_SUBMENU_LIST[selectedSubMenuIdx].children.length > 0">
                                    ...
                                    <ul
                                        ...
                                        <template v-for="  child   in   subMenu.children  ">
                                            <li class="text-body1 cursor-pointer hover:text-primary-400"
                                                @click=" onItemClick(`/smartservice/${subMenu.title_route}/${child.title_route}`)">
                                                {{ child.title }}
                                            </li>
                                        </template>
                                    </ul>
                ...
                <div
                    ...
                    <img :src="SMARTSERVICE_SUBMENU_LIST[selectedSubMenuIdx].img" class="w-[160px] md:w-[200px] h-[94px] md:h-[120px]">
                    <div class="ml-[14px] md:ml-[34px]">
                        <p class="text-[15px] md:text-[18px] leading-[20px] md:leading-[24px] mb-[10px]">{{
                            SMARTSERVICE_SUBMENU_LIST[selectedSubMenuIdx].title }}</p>
                        <p class="text-[13px] md:text-[15px] leading-[18px] md:leading-[20px]">{{
                            SMARTSERVICE_SUBMENU_LIST[selectedSubMenuIdx].desc }}</p>
                    </div>
                </div>
